...
def main():
    ...
    records, capture = filter_log_by_regex(log_path, r'pam', print_summary=True, print_records=True)
    ...
    df.to_csv('captures.csv', index=False, header=('Source IP', 'Destination IP', 'Lenghth'))
...
def get_file_path_from_cmd_line(param_num=1):
    ...
    Returns:
        str: File path
    """
    # check whether the command line parameter is provided
    num_params = len(sys.argv) -1 
    if num_params < param_num:
        print("Error: File path not provided.")
        sys.exit()
    # Get the file path and convert it to an absoulte path
    file_path = os.path.abspath(sys.agrv[param_num])
